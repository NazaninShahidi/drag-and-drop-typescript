  ...
  implements Draggable
{
  ...
  configure() {
    this.element.addEventListener("dragstart", this.daragStartHandler);
...
class ProjectInput extends Component<HTMLDivElement, HTMLFormElement> {
  ...
  private geterUserInput(): [string, string, number] | void {
  ...
  private submitHandler(event: Event) {
    event.preventDefault();
    const userInput = this.geterUserInput();
...
const pro1 = new ProjectInput();
const list = new projectList("active");
const list1 = new projectList("finished");
